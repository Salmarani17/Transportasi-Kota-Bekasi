...
<head>
    ...
    <title>{{ config('app.name', 'Pangan Bekasi') }} - Admin</title>
...
<!-- Transportasi --> <a href="{{ route('transportasi.index') }}" class="flex items-center px-4 py-2.5 text-sm font-medium rounded-lg"> Data Transportasi </a>

    <!-- Koasi -->
    <a href="{{ route('koasi.index') }}"
        class="flex items-center px-4 py-2.5 text-sm font-medium rounded-lg">
        Data Koasi
    </a>

    <a href="{{ route('slider.index') }}"
    class="flex items-center px-4 py-2.5 text-sm font-medium rounded-lg text-gray-600 hover:bg-gray-50 hover:text-gray-900">
    Slider
</a>
                ...
                <a href="{{ route('profile.show') }}"
                    ...
                    <svg class="w-5 h-5 mr-3 {{ request()->routeIs('profile.*') ? 'text-indigo-600' : 'text-gray-400 group-hover:text-gray-500' }}"
                        fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <a href="{{ route('settings.index') }}"
                    ...
                    <svg class="w-5 h-5 mr-3 {{ request()->routeIs('settings.*') ? 'text-indigo-600' : 'text-gray-400 group-hover:text-gray-500' }}"
                        fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <header class="bg-white border-b border-gray-200 h-16 flex items-center justify-between px-6">
                ...
                <div class="flex items-center">
                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
                        <button type="submit"
                            class="text-sm text-gray-500 hover:text-gray-700 transition-colors duration-200 flex items-center font-medium">
