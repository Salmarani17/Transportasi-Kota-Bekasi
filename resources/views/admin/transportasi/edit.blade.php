<x-admin-layout>
    ...
    <form action="{{ route('transportasi.update', $transportasi->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <select name="jenis" class="w-full border p-2 mb-3 rounded">
            ...
            <option value="trans_bekasi" {{ $transportasi->jenis == 'trans_bekasi' ? 'selected' : '' }}>Trans Bekasi</option>
            <option value="transjakarta" {{ $transportasi->jenis == 'transjakarta' ? 'selected' : '' }}>Transjakarta</option>
        </select>
